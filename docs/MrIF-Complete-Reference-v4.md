# Mr.IF (chaos2alpha) — Complete Reference Document v4.0
...
└──────────┬───────────────────┘
           ...
           ▼
┌──────────────────────────────────────────────┐
│  LLM Internal Reasoning (guided by 9 Skills) │
│  event-classification → Step 0 classify      │
...
### 8. novel-event-reasoning.md — First-Principles Novel Event Analysis (NEW v3.2)

- **Purpose**: Guides FIRST-PRINCIPLES reasoning for novel events that have zero keyword matches
- **Core Framework: Follow the Money**: 5-step process (Who pays? → Who earns? → Revenue materiality → Priced in? → Time horizon)
- **Revenue Materiality Quick Reference**: Size benchmarks for different impact scales
- **Anti-Patterns**: No vague sector calls, no ignoring "already priced in", no single-chain thinking
- **Integration**: Triggered when mr_if_reason detects NOVEL EVENT (zero keyword matches and no LLM classification)

### 9. event-classification.md — LLM-Guided Event Classification (NEW v4.0)

- **Purpose**: Teaches the LLM to semantically classify events BEFORE calling `mr_if_reason`, replacing keyword dependency
- **12 Event Types**: Complete decision logic for each type with examples and disambiguation rules
- **Classification Decision Logic**: 4-step process (Financial vs Daily-life → Handle ambiguity → Handle novel events → Extract entities)
- **Novel Event Mapping Table**: Maps common novel event categories (Olympics, elections, scandals, disasters) to appropriate types
- **15+ Classification Examples**: Covering daily, financial, geopolitical, and novel event inputs
- **Integration**: Used in Step 0 of the workflow — LLM classifies, then passes `event_type` to `mr_if_reason`
- **Key Innovation**: Eliminates the keyword coverage gap — LLM semantic understanding covers events that no keyword list ever could

---

## LLM-Guided Event Classification (NEW v4.0)

The most significant architectural change in v4: **event classification is no longer solely keyword-driven**.

### Problem (v3.x)
- Internal `classifyEvent()` uses regex keyword matching against ~200 hardcoded keywords
- Novel events (Olympics, elections, scandals, disasters) with no matching keywords fall through to `daily` type
- Fallback templates are generic; the reasoning scaffold underserves these inputs
- Adding new keywords is a maintenance burden that never converges

### Solution (v4.0)
- **Step 0** added to workflow: LLM semantically classifies the event BEFORE calling the tool
- `mr_if_reason` now accepts an optional `event_type` parameter
- When `event_type` is provided: overrides internal keyword classification, uses correct templates/anchors
- When `event_type` is omitted: falls back to keyword matching (backward compatible)
- Novel event detection only triggers when BOTH keywords fail AND LLM didn't classify

### Architecture Flow
```
User input → LLM reads event-classification skill → determines event_type
           → LLM calls mr_if_reason(user_input, event_type="geopolitical")
           → Tool uses "geopolitical" as primary_type (skips keyword matching for primary)
           → Keywords still run for secondary_types and template trigger matching
           → Correct templates, anchors, and discipline knowledge are returned
```

### Benefits
1. **Coverage**: Any event the LLM understands gets correct classification (no keyword gap)
2. **Ambiguity**: LLM can disambiguate ("Fed降息" → `market_event`, not `economic`)
3. **Maintenance**: Zero keyword additions needed for new event types
4. **Multi-language**: Works in any language the LLM understands
5. **Backward compatible**: Omitting `event_type` preserves v3.x behavior
...
| Skills | 9 (+event-classification, novel-event-reasoning v4.0) |
...
| Reasoning modes | 3 (BUTTERFLY EFFECT, FINANCIAL TRANSMISSION, FIRST-PRINCIPLES) |
...
| Classification method | LLM-driven (v4) + keyword fallback |

---

*Document generated: 2026-02-09 | Version: 4.0 | Project: chaos2alpha (Mr.IF)*
